...
const PL_STAGES = [
  ...
  { id: "projekt",     label: "Projekt rzÄdowy",   short: "PROJ",     color: "#8b5cf6" },
...
const EU_STAGES = [
  ...
  { id: "przyjeta",     label: "PrzyjÄta",           short: "PRZYJ", color: "#f59e0b" },
  { id: "transpozycja", label: "Transpozycja do PL", short: "TRANS", color: "#f97316" },
  { id: "obowiazuje",   label: "ObowiÄzuje",         short: "OBO",   color: "#22c55e" },
];
const TOPICS = [
  { id: "all",         label: "Wszystkie",    icon: "âĄ" },
  { id: "payments",    label: "PĹatnoĹci",    icon: "đł" },
  { id: "crypto",      label: "Krypto/DeFi",  icon: "âż"  },
  { id: "aml",         label: "AML/KYC",      icon: "đ" },
  { id: "lending",     label: "Kredyt",       icon: "đŚ" },
  { id: "openbanking", label: "Open Banking", icon: "đ" },
  { id: "ai",          label: "AI/Dane",      icon: "đ¤" },
  { id: "dora",        label: "Cyber/DORA",   icon: "đĄď¸" },
];
const UM = {
  high:   { l: "Pilne",    c: "#ef4444" },
  medium: { l: "WaĹźne",    c: "#f59e0b" },
...
const fmtDate     = d => d ? new Date(d).toLocaleDateString("pl-PL", { day: "numeric", month: "short", year: "numeric" }) : "â";
const uid         = () => Math.random().toString(36).slice(2, 8);

const API_KEY = process.env.REACT_APP_ANTHROPIC_API_KEY || "";

const SYS = `JesteĹ ekspertem regulacji finansowych fintech PL i EU. Odpowiadasz TYLKO jako JSON tablica, zero markdown, zero komentarzy.
Schemat kaĹźdego obiektu:
{"id":"kebab-case","title":"peĹna nazwa aktu","shortTitle":"max 4 sĹowa","region":"PL|EU","topic":"payments|crypto|aml|lending|openbanking|ai|dora","icon":"emoji","stage":"id_etapu","stageChangedAt":"YYYY-MM-DD","deadline":"YYYY-MM-DD|null","urgency":"high|medium|low","summary":"2-3 zdania o treĹci","impact":"konkretny wpĹyw na fintech","nextStep":"nastÄpny krok i kiedy","source":"organ/instytucja"}
...
ZwrĂłÄ 10-12 rzeczywistych, aktualnych projektĂłw. Dzisiaj: ${new Date().toISOString().split("T")[0]}.`;
...
export default function App() {
    ...
    try {
      ...
      parsed.forEach(item => {
        ...
        if (p && p.stage !== item.stage) {
          newA.push({ id: uid(), itemId: item.id, icon: item.icon, title: item.shortTitle, msg: `${getStageObj(item.region, p.stage).label} â ${getStageObj(item.region, item.stage).label}` });
        }
      });
  ...
  const exportPDF = () => {
    ...
    const rows = filtered.map(i => {
      ...
      return `<tr><td>${i.icon} ${i.title}</td><td style="color:${i.region === "PL" ? "#1d4ed8" : "#7c3aed"};font-weight:700">${i.region}</td><td>${st.label}</td><td style="color:${UM[i.urgency]?.c}">${UM[i.urgency]?.l || ""}</td><td>${i.deadline ? fmtDate(i.deadline) : "â"}</td><td style="color:#666;font-size:11px">${notes[i.id] || "â"}</td></tr>`;
    }).join("");
    const w = window.open("", "_blank");
    w.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Raport â Fintech Poland</title>
    ...
    <h1>âď¸ Monitor Legislacyjny</h1><div class="sub">Fintech Poland Âˇ ${now} Âˇ ${filtered.length} projektĂłw</div>
    <div class="stats">
    ...
    <p style="margin-top:28px;color:#94a3b8;font-size:11px;text-align:center">Fintech Poland Âˇ Monitor Legislacyjny Âˇ AI-powered</p>
    </body></html>`);
  ...
  if (showKeyInput) return (
    <div style={{ minHeight: "100vh", background: "#040b16", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'DM Sans', sans-serif" }}>
      <div style={{ background: "#060d1c", border: "1px solid #1a2744", borderRadius: 16, padding: "40px 36px", maxWidth: 420, width: "90%" }}>
        <div style={{ fontSize: 40, marginBottom: 16, textAlign: "center" }}>âď¸</div>
        ...
        <p style={{ color: "#475569", fontSize: 13, textAlign: "center", marginBottom: 28, lineHeight: 1.6 }}>
          Podaj klucz API Anthropic aby uruchomiÄ wyszukiwanie legislacyjne zasilane AI.
        </p>
        ...
        <p style={{ color: "#1e2d4a", fontSize: 11, textAlign: "center", marginTop: 16, lineHeight: 1.6 }}>
          Klucz jest przechowywany tylko lokalnie w przeglÄdarce.<br />
          Nie masz klucza? ZdobÄdĹş go na <span style={{ color: "#3b82f6" }}>console.anthropic.com</span>
        </p>
      </div>
    </div>
  );
    ...
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", background: "#040b16", fontFamily: "'DM Sans', 'Segoe UI', sans-serif", color: "#dde8f8", overflow: "hidden" }}>
      ...
      <div style={{ background: "#060d1c", borderBottom: "1px solid #0c1828", padding: "0 16px", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, height: 52 }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="đ szukaj projektu..."
          ...
          <button onClick={exportPDF} style={{ background: "#041009", border: "1px solid #14532d44", borderRadius: 6, color: "#22c55e", padding: "5px 10px", fontSize: 10, fontFamily: "'Space Mono', monospace", fontWeight: 700 }}>â PDF</button>
          ...
          <button onClick={() => load()} disabled={loading} style={{ background: "#09111f", border: "1px solid #0c1828", borderRadius: 6, color: "#334155", padding: "5px 10px", fontSize: 13 }}>
            <span style={{ display: "inline-block", animation: loading ? "spin .7s linear infinite" : "none" }}>âť</span>
          </button>
      ...
      {alertsOpen && (
        <div style={{ position: "fixed", top: 58, right: 14, width: 300, background: "#060d1c", border: "1px solid #1a2744", borderRadius: 10, zIndex: 100, padding: "12px", boxShadow: "0 20px 60px #000000aa" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            <div style={{ fontSize: 8, color: "#f59e0b", fontFamily: "'Space Mono', monospace", letterSpacing: "0.12em" }}>đ ALERTY LEGISLACYJNE</div>
            <div style={{ display: "flex", gap: 8 }}>
              {alerts.length > 0 && <button onClick={() => setAlerts([])} style={{ background: "none", border: "none", color: "#334155", fontSize: 10 }}>WyczyĹÄ</button>}
              <button onClick={() => setAlertsOpen(false)} style={{ background: "none", border: "none", color: "#334155", fontSize: 15 }}>Ă</button>
            </div>
          </div>
          {alerts.length === 0
            ? <div style={{ textAlign: "center", padding: "18px", color: "#1a2744", fontSize: 12 }}>Brak alertĂłw</div>
            : alerts.slice(0, 6).map(a => (
              <div key={a.id} style={{ display: "flex", gap: 8, padding: "8px 10px", background: "#09111f", borderRadius: 7, marginBottom: 5, border: "1px solid #1a2744" }}>
                ...
                <button onClick={() => setAlerts(al => al.filter(x => x.id !== a.id))} style={{ background: "none", border: "none", color: "#334155", fontSize: 13 }}>Ă</button>
              </div>
            ))
          }
        </div>
      ...
      {!loading && urgent.length > 0 && (
        <div style={{ background: "#0e0508", borderBottom: "1px solid #7f1d1d22", padding: "4px 14px", display: "flex", gap: 7, alignItems: "center", overflowX: "auto", flexShrink: 0 }}>
          <span style={{ fontFamily: "'Space Mono', monospace", fontSize: 8, color: "#ef4444", letterSpacing: "0.12em", whiteSpace: "nowrap" }}>â  PILNE</span>
          {urgent.map(i => (
      ...
      {loading ? (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 14 }}>
          ...
          <div style={{ color: "#1a2744", fontSize: 13 }}>PrzeszukujÄ ĹşrĂłdĹa legislacyjne...</div>
        </div>
      ) : error ? (
        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ background: "#0e0508", border: "1px solid #7f1d1d", borderRadius: 12, padding: "24px 28px", color: "#fca5a5", textAlign: "center", maxWidth: 360 }}>
            <div style={{ fontSize: 34, marginBottom: 10 }}>â ď¸</div>
            <div style={{ fontSize: 13, marginBottom: 14, lineHeight: 1.6 }}>{error}</div>
            <button onClick={() => load()} style={{ background: "#3b82f6", border: "none", borderRadius: 6, color: "white", padding: "8px 20px", fontSize: 12, fontWeight: 600 }}>SprĂłbuj ponownie</button>
            <button onClick={() => setShowKeyInput(true)} style={{ display: "block", width: "100%", marginTop: 8, background: "transparent", border: "1px solid #334155", borderRadius: 6, color: "#64748b", padding: "8px 20px", fontSize: 12 }}>ZmieĹ klucz API</button>
          </div>
        </div>
      ) : view === "list" ? (
        <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
          ...
          <div style={{ width: 300, flexShrink: 0, borderRight: "1px solid #0c1828", overflowY: "auto", padding: "8px" }}>
            {filtered.length === 0
              ? <div style={{ textAlign: "center", padding: "40px", color: "#1a2744", fontSize: 12 }}>Brak wynikĂłw</div>
              : filtered.map(item => {
                    ...
                    <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: "flex", gap: 3, flexWrap: "wrap", marginBottom: 4 }}>
                          ...
                          {item.deadline && (() => { const d = daysUntil(item.deadline); if (d === null) return null; const c = d < 0 ? "#64748b" : d < 30 ? "#ef4444" : d < 90 ? "#f59e0b" : "#22c55e"; return <span style={{ fontSize: 9, fontWeight: 700, padding: "1px 5px", borderRadius: 3, background: c + "1a", color: c, border: `1px solid ${c}33`, fontFamily: "monospace" }}>{d < 0 ? "MINÄĹ" : d + "d"} âą</span>; })()}
                        </div>
                        <div style={{ color: "#dde8f8", fontWeight: 600, fontSize: 12, lineHeight: 1.4, marginBottom: 2 }}>{item.shortTitle}</div>
                        <div style={{ color: "#2d3f58", fontSize: 10 }}>{item.source}{ds !== null && <span style={{ color: ds > 45 ? "#1e2d4a" : "#f59e0b" }}> Âˇ {ds}d</span>}</div>
          ...
          <div style={{ flex: 1, overflowY: "auto" }}>
            {!selected ? (
              <div style={{ height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 10 }}>
                <div style={{ fontSize: 48 }}>âď¸</div>
                <div style={{ color: "#111d30", fontSize: 12, textAlign: "center", lineHeight: 1.8 }}>Wybierz projekt<br />aby zobaczyÄ szczegĂłĹy</div>
              </div>
                ...
                <div style={{ flex: 1, overflowY: "auto", padding: "6px" }}>
                  {col.map(item => (
                    <div key={item.id} className="hov" onClick={() => { setSelected(item); setView("list"); }}
                      ...
                      {notes[item.id] && <div style={{ marginTop: 4, fontSize: 8, color: "#34d399", fontFamily: "monospace" }}>đ notatka</div>}
                    </div>
                  ))}
                  {col.length === 0 && <div style={{ textAlign: "center", padding: "16px 0", color: "#0c1828", fontSize: 16 }}>Âˇ</div>}
                </div>
              </div>
            );
          })}
  ...
  return (
    <div style={{ padding: "18px", animation: "sr .22s ease both" }}>
          ...
          return (
            <div key={s.id} style={{ display: "flex", alignItems: "flex-start", gap: 9 }}>
              ...
              <div style={{ flex: 1, padding: "2px 9px", borderRadius: 5, marginBottom: 1, background: active ? s.color + "12" : "transparent", border: `1px solid ${active ? s.color + "33" : "transparent"}` }}>
                ...
                {active && item.stageChangedAt && <div style={{ color: "#334155", fontSize: 9, marginTop: 1 }}>od {fmtDate(item.stageChangedAt)} Âˇ {daysSince(item.stageChangedAt)}d temu</div>}
              </div>
            </div>
          );
        })}
      ...
      <div style={{ background: "#060c18", border: `1px solid ${st.color}33`, borderLeft: `3px solid ${st.color}`, borderRadius: 9, padding: "12px", marginBottom: 9 }}>
        <div style={{ fontSize: 8, color: st.color, fontFamily: "monospace", letterSpacing: "0.12em", marginBottom: 7 }}>WPĹYW NA FINTECH</div>
      ...
      {item.nextStep && <div style={{ background: "#040e08", border: "1px solid #14532d44", borderLeft: "3px solid #22c55e", borderRadius: 9, padding: "12px", marginBottom: 9 }}>
        <div style={{ fontSize: 8, color: "#22c55e", fontFamily: "monospace", letterSpacing: "0.12em", marginBottom: 7 }}>NASTÄPNY KROK</div>
      ...
      {item.deadline && <div style={{ background: "#060c18", border: "1px solid #111d30", borderRadius: 9, padding: "11px 14px", marginBottom: 9, display: "flex", gap: 10, alignItems: "center" }}>
        ...
        <div>
          <div style={{ fontSize: 8, color: "#1e2d4a", fontFamily: "monospace", letterSpacing: "0.1em" }}>TERMIN WEJĹCIA W ĹťYCIE</div>
      ...
      <div style={{ background: "#060c18", border: "1px solid #111d30", borderRadius: 9, padding: "12px" }}>
        <div style={{ fontSize: 8, color: "#1e2d4a", fontFamily: "monospace", letterSpacing: "0.12em", marginBottom: 9 }}>đ NOTATKI ZESPOĹU</div>
        <textarea value={noteVal} onChange={e => setNoteVal(e.target.value)} placeholder="Przypisz osobÄ, dodaj komentarz, zaznacz priorytet..." rows={3}
        ...
        <button onClick={saveNote} style={{ marginTop: 7, background: "#1e3a5f", border: "1px solid #2563eb44", borderRadius: 5, color: "#60a5fa", padding: "5px 13px", fontSize: 10, fontWeight: 600 }}>
          {noteSaved ? "â Zapisano" : "Zapisz notatkÄ"}
        </button>
      </div>
    </div>
  );
